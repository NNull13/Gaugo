...
func TestEvaluateJSONInvalidConfig(t *testing.T) {
	...
	_, err := New(Config{})
	if err == nil || !strings.Contains(err.Error(), "invalid anthropic config") {
...
func TestValidateURLPolicy(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			cfg: Config{
				APIKey:  "k",
				BaseURL: "https://user:pass@api.anthropic.com",
			},
			wantErr: true,
		},
		{
			...
			cfg: Config{
				APIKey:         "k",
				EndpointURL:    "http://127.0.0.1:8080/v1/messages",
				AllowUnsafeURL: true,
			},
		},
		{
			...
			cfg: Config{
				APIKey:         "k",
				EndpointURL:    "ftp://127.0.0.1:8080/v1/messages",
				AllowUnsafeURL: true,
			},
			wantErr: true,
		},
...
func TestEvaluateJSON(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"model":"claude-test","content":[{"type":"text","text":"{\"ok\":true}"}]}`))
	}))
	defer srv.Close()
