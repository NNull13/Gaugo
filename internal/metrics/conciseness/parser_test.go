...
func TestParse(t *testing.T) {
	...
	out, err := Parse([]byte(`{"score":0.8,"issues":[{"text":"repetition","reason":"same point repeated"}],"reason":"mostly concise"}`))
	if err != nil {
	...
	if len(out.Issues) != 1 || out.Issues[0].Text != "repetition" || out.Issues[0].Reason != "same point repeated" {
		t.Fatalf("unexpected parsed issues: %+v", out.Issues)
	}
	if out.Reason != "mostly concise" {
		t.Fatalf("reason got=%q want=mostly concise", out.Reason)
	}
}
